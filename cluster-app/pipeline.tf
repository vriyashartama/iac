...
data "kubectl_path_documents" "traefik-count-hack" {
  pattern = "${path.module}/manifest/traefik/*.yaml"
  vars = {
...
resource "kubectl_manifest" "traefik" {
  count     = length(data.kubectl_path_documents.traefik-count-hack.documents)
...
data "kubectl_path_documents" "default-count-hack" {
  pattern = "${path.module}/manifest/*.yaml"
  vars = {
      azurestorageaccountname = ""
      azurestorageaccountkey = ""
...
resource "kubectl_manifest" "default" {
  count     = length(data.kubectl_path_documents.default-count-hack.documents)
